...
public class WeightedGraph {
	...
	public void insertEdge(int src,int dest,int weight) {
		...
		Edge edgeS = new Edge(src,dest,weight);
		Edge edgeN = new Edge(dest,src,weight);
		if(!srcSet.contains(edgeS) && !destSet.contains(edgeN)) {
		  srcSet.add(edgeS);
		  destSet.add(edgeN);
		  map.put(src, srcSet);
	...
	public void insertEdge(Edge edge) {
		...
		list.add(edge);
		map.put(src,list);
		Edge edgeN = new Edge(edge.dest,edge.src,edge.weight);
		if(!map.containsKey(dest)) {
			map.put(dest, new HashSet<Edge>());
			verticesCount++;
	    }
	...
	public static void printGraph(WeightedGraph sT) {
		System.out.println();
